...
Eine weitere. Sie kennen das bestimmt aus Ihrer Bachelorarbeit (oder von einem anderen Analyseprojet), dass unterschieliche Daten in unterschiedlichen Datein vorliegen. Häufig liegen verschieden Angaben zu den Versuchspersonen (Alter, Abschluss, etc.) in einer Datei vor und die experimentelle Daten (Fragebögen, Reaktionszeiten, etc.) in anderen, separaten, Dateien.

Für unsere Analysen ist es dann häufig hilfreich, ween diese Dateien kurzfristig "zusammengefügt" werden könnten. Somit können wir Beziehungen zwischen beispielsweise Personenvariablen und Kennwerte aus einer Untersuchung aufdecken. Dafür ist es notwending, dass in beiden Datensätzen ein **Persornenidentifier** (`id`) vorhanden ist. Dieser müss für jede Person einzigartig sein, sonst können wir nicht eindeutig Beobachtungen zu Personen zuordnen.

Glücklicherweise können wir Datensätze mit `dplyr` zusammen fügen. Diese operation nennt man bei **join** (verbinden). Bei `dplyr` gibt es zwei verschiedene `joins`, die wir gerne heute anschauen wollen, *inner_join* und *outer_join*.

## `inner_join`

`dplyr` enthält die Funktion `inner_join()`. Die Funktion bekommt als Argumente zwei Datensätze (`x` und `y`) und den Persoenidentifier (`by`). Das Ergebnis eines `inner_join` enthält alle Spalten von `x` und `y` sowie alle Zeilen, für die es einen Match in beiden Datensätze gibt. Fälle die nicht in beiden Datensätzen vorkommen, tauchen nicht im Ergebnis eines `inner_join` auf. Es ist daher immer Vorsicht angebracht.

Als einfaches Beispiel verwenden wir hier eine Tabelle mit Daten über das Alter von zehn Person und eine Tabelle mit Daten über ihren Wohnort:

Erstellen wir ersteimal die `altersdaten`:
...
wohnortdaten <- data.frame(
  wohnort = c('Mainz', 'Frankfurt', NA, 'Berlin', 'Wiesbaden', 'Mainz', 'München'),
  id = c(1, 2, 3, 4, 5, 6, 8))

wohnortdaten
```

**Achtung!**: Schauen Sie sich beide Datensätze genau an. Vielleicht ist es Ihnen aufgefallen, dass wir im Datensatz `wohnortdaten` eine Person 8 haben und keine Person 7. Im Datensatz `altersdaten` haben wir dafür eine Person 7 aber keine Person 8. Zusätzlich haben wir in einem Datensatz fehlende Werte (kein Wohnort für Person 3).

Benutzen wir jetzt ein `inner_join`, um die Datensätze zusammenzufügen.
`x` ist unser Datensatz `altersdaten` und `y` ist unser Datensatz `wohnortdaten`. Im Argument `by` setzen wir den Personenidentifier `id`.

```{r}
inner_join_daten <- inner_join(x = altersdaten, y = wohnortdaten, by = 'id')

inner_join_daten
```

Die `inner_join()` Funktion gibt uns einen weiten Datensatz in dem sich die Personen befinden, die in beiden Datensätze einen Eintrag haben. Dagegen sind Person 7 und Person 8 nicht im "Ergebnis"-Datensatz vorhanden. Das hat zum Grund, dass Person 7 nur im ersten Datensatz vorkommt und Person 8 nur im zweiten.

Verändern wir nun etwas den Datensatz `altersdaten`. Die Variable `id` werden wir nun `VP` nennen.
...
Zuvor haben im Argument `by` im `inner_join` einfach der Name der ID-Variable schreiben müssen. Diese hatte im vorherigen Beispiel den gleichen Namen im beiden Datensätze. `inner_join` hat dann automatisch in beiden Datensätze geschaut, diese Variable gefunden und die Daten der Personen anhand ihrer ID zusammengefügt. 

Was machen wir allerdings, wenn die ID-Variable unterschidliche Namen hat in beiden Datensätzen?
Glücklicherweise hat `inner_join` hierfür eine Läsung parat. Wir können im Argument `by`, anstatt der Name der ID-Variable, eine Gleichung aufgnehmen, die die 
